...
def visit_create(request, theme_pk):
    ...
    # 지금 누른 식당이 그 유저한테 인증되있는지 확인
    CVisit = AchieveUser.objects.create(count=theme_pk, user=user, receipt=request.data['receipt'])
    # 가게 위치와 내 위치 -> 현재 theme store data에 위치 정보를 등록하지 않았다....
    
    #이미지 경로
    # image_path = CVisit.receipt
    image_path = request.data['receipt']
    #검증할 음식점 이름
    ...
    if chk:
        if found :
            ...
            # 여기에 DB에 저장하는 로직
            AchieveUser.objects.create(count=theme_pk, user=user, receipt=request.data['receipt'])
